...
File:    17_mechanism_check_sexual_debut.do

Course:  ECON421 — Education and Human Capital in Developing Economies
Author:  Erika Salvador '28 (esalvador28@amherst.edu)

Purpose:
  Mechanism check: sexual initiation.

  This file estimates whether SHS exposure shifts (i) the likelihood of
  ever having had sex and (ii) the timing of sexual debut, using the
  same DiD framework as the main fertility-timing results.
...
*--------------------------------------------------------------*
* 4. Outcomes (two panels)
*--------------------------------------------------------------*
local outcomes "ever_sex age_first_sex"

*--------------------------------------------------------------*
* 5. Run regressions & store coef/SE/stars + fit stats by outcome
*--------------------------------------------------------------*
local row = 0
...
*--------------------------------------------------------------*
* 6. Write LaTeX: two panels (each with own controls + fit stats)
*--------------------------------------------------------------*
capture file close fh
file open fh using "$TABLE_D2/mech_sexualdebut.tex", write replace

file write fh "\begin{table}[htbp]" _n
file write fh "\centering" _n
file write fh "\renewcommand{\arraystretch}{0.92}" _n
file write fh "\caption{Sexual initiation as a mechanism}" _n
file write fh "\label{tab:mech_sexualdebut}" _n
file write fh "\begin{tabular}{@{}lccc@{}}" _n
file write fh "\toprule" _n

*-------------------------------*
* Panel A: Ever had sex
*-------------------------------*
...
file write fh "\textit{Notes:} Standard errors in parentheses, clustered at the province level. All regressions include province, birth-cohort, and survey-year fixed effects and are weighted using DHS sampling weights, and use the same set of individual controls as in the main specification (urban residence, household wealth index, religion, and ethnicity), added sequentially across columns. The dependent variable in the first panel is an indicator for ever having had sex; the second panel reports age at first sexual intercourse (in completed years). *, **, *** denote significance at the 10\%, 5\%, and 1\% levels." _n
file write fh "}" _n

file write fh "\end{table}" _n
file close fh

display as text "Done: two-panel sexual initiation mechanism table written to $TABLE_D2/mech_sexualdebut.tex"
